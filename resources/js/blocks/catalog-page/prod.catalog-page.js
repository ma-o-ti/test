...
import { loader, sendUrl } from "../../api/ajax-function";
import { decodeCommaUrl } from "../../api/decode";
...
ready(() => {
  ...
  if (catalogParent !== false) {
     ...
     */
    async function addProductsCatalogList(parentWrap, path) {
      loader(true, "body");
      try {
        let response = await fetch(`${productsUrlItems}${path}`);
        let newData = await response.json();
        if (newData) {
          parentWrap.querySelector(".catalog-list").innerHTML += newData.products;
          //обновляем пагинацию если она есть
          if (path == "catalog" || path == "catalog-piece") {
            parentWrap.querySelector(".pagination").remove();
            parentWrap.insertAdjacentHTML("beforeend", newData.pagination);
          }

          //удаляем "Показать еще" если больше нет товаров(true если больше нет товаров)
          if (newData.showAllProducts) {
            parentWrap.querySelector(".js-show-more-products").remove();
          }
    ...
    const deleteCatalogItem = (event) => {
      ...
      if (deleteBtn !== false) {
        ...
        let id = 1; // подставляем нужное значение для обращения к БД.
